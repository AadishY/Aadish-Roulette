...
export const SettingsMenu: React.FC<SettingsMenuProps> = ({ settings, onUpdateSettings, onClose, onResetDefaults }) => {
    ...
    return (
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="w-full max-w-md bg-stone-900 border-2 border-stone-600 shadow-[0_0_50px_rgba(0,0,0,1)] p-6 md:p-8 relative clip-path-slant flex flex-col max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-6 flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                        ...
                        <button 
                            onClick={() => handleChange('vhsFilter', !settings.vhsFilter)}
                            className={`p-3 border-2 flex flex-col items-center gap-2 transition-all ${settings.vhsFilter ? 'border-red-600 bg-red-900/20 text-white' : 'border-stone-700 bg-stone-800 text-stone-500'}`}
                        >
                            <Tv size={20} />
                            <span className="text-xs font-bold tracking-wider">VHS FILTER</span>
                            <span className="text-[10px]">{settings.vhsFilter ? 'ON' : 'OFF'}</span>
                        </button>
                    </div>
                </div>
